      ...
      <ul>
        {sampleTrips.map(trip => (
          <li key={trip.id}>
            ...
            <div>
              <strong>Activities ({trip.activities.length}):</strong>
              <ul>
                {trip.activities.map(activity => (
                  <li key={activity.id} style={{marginLeft: '20px', marginTop: '5px'}}>
                    <strong>{activity.name}</strong> ({activity.category}) - ${activity.cost}
                    <br />
                    <small>📍 {activity.location} • ⏱️ {activity.duration} • 👍 {activity.votes} votes</small>
                  </li>
                ))}
              </ul>
            </div>
