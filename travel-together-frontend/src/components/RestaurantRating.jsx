...
const RestaurantRating = ({ restaurant, participants, onRatingChange, tripId, ratings = {}, onEditRestaurant, onDeleteRestaurant }) => {
  ...
  const handleSaveEdit = async () => {
    try {
      if (onEditRestaurant) {
        await onEditRestaurant(restaurant.id, editFormData);
      }
      setIsEditing(false);
    } catch (error) {
    ...
    <div className={`border rounded-lg p-4 sm:p-6 mb-4 transition-all duration-300 hover:shadow-md ${currentConfig.bgColor} ${currentConfig.borderColor}`}>
      ...
      {isEditing && (
        <div className="mb-6 p-4 bg-white border border-gray-200 rounded-lg">
          <div className="flex justify-between items-center mb-4">
            <h5 className="text-lg font-semibold text-gray-900">Edit Restaurant</h5>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setEditFormData(prev => ({ ...prev, priceRange: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-orange-500"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setEditFormData(prev => ({ ...prev, groupCapacity: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
          </div>
      ...
      <div>
        ...
        {isLoggedIn && participants.includes(loggedInUser) && participants.filter(p => p === loggedInUser).map(participant => {
          ...
          return (
            <div key={participant} className="bg-orange-50 border-2 border-orange-200 bg-opacity-90 p-3 rounded-lg backdrop-blur-sm mb-4">
              <span className="text-sm font-medium text-orange-900 mb-2 block">
                {participant} (You)
              </span>
              <div className="w-full">
                ...
                <div className="text-center sm:text-left text-xs font-semibold text-orange-700">
