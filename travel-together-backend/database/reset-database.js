...
async function confirmReset() {
  ...
  return new Promise((resolve) => {
    rl.question('⚠️  This will DELETE ALL current trip data and reset to sample data. Continue? (y/n): ', (answer) => {
      rl.close();
...
async function backupDatabase() {
  ...
  const backupPath = path.join(__dirname, `traveltogether_backup_${timestamp}.db`);
  const currentPath = path.join(__dirname, 'traveltogether.db');
...
async function resetDatabase() {
  try {
    ...
    for (const tripData of sampleTrips) {
      console.log(`Resetting trip: ${tripData.name}`);
    ...
    console.log(`\n📊 Reset to ${trips.length} sample trips:`);
    trips.forEach(trip => {
...
module.exports = { resetDatabase, sampleTrips };